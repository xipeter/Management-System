	...
	public class tvDeptList :System.Windows.Forms.TreeView 
	{
		...
		private void RefreshList()
		{
			...
			for(int i=0 ;i<this.myDepts.Count;i++)
			{
				...
				//类型为叶
				if(this.myDepts[i].GetType()==typeof(Neusoft.HISFC.Models.Base.Department))//科室
				{
					try
					{
						Neusoft.HISFC.Models.Base.Department Info=(Neusoft.HISFC.Models.Base.Department)this.myDepts[i];
						obj.ID = Info.ID;
						obj.Name = Info.Name;
						try
						{
				...
				else if(this.myDepts[i].GetType().ToString()=="Neusoft.FrameWork.Models.NeuObject")
				{
					obj = (Neusoft.FrameWork.Models.NeuObject)this.myDepts[i];
