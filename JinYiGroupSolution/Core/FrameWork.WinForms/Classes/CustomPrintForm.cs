...
namespace Neusoft.FrameWork.WinForms.Classes
{
	...
	public class CustomPrintForm
	{
		...
		*/
		...
		public static void AddCustomPaperSizeToDefaultPrinter(string paperName, float widthMm, float heightMm,bool reset)
		{
			string sPrinterName = pd.PrinterSettings.PrinterName;
			CustomPrintForm.isReset = reset;
		...
		private static bool HavePage(string name)
		{
			//
			//查找最符合的纸张
			//
			...
			widthMm, float heightMm)
		{
			...
			//有这个页了，不重新设置
			if(HavePage(paperName) && isReset==false) return;
