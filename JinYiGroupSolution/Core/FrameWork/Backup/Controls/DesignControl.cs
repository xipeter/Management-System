...
namespace Neusoft.FrameWork.WinForms.Controls
{
    ...
    public sealed class DesignControl : System.ComponentModel.Component
    {
        ...
        private Type[] TypeChangeText
        {
            get
            {
                if (typeChangeText == null)
                {
                    typeChangeText = new Type[ControlChangeText.Length];
                    for (int i = 0; i < this.controlChangeText.Length; i++)
                    {
                        typeChangeText[i] = OwnAssembly.GetType(controlChangeText[i]);
                    }
                }
        ...
        public Control CurrentControl
        {
            ...
            {
                if (propertyform != null)
                    propertyform.ControlToChange = value;
                curControl = value;
            }
            get
            {
        ...
        public bool IsDesignMode
        {
            ...
            set
            {
                ...
                {
                    if (isShowTip)
                        Neusoft.FrameWork.WinForms.Classes.Function.ShowToolTip("当前为设计模式(工具在任务栏里)", 5);
                ...
                {
                    if (isShowTip)
                        Neusoft.FrameWork.WinForms.Classes.Function.ShowToolTip("当前已经转换为正常模式", 1);
        ...
        private void AddKeyDown(Control parentControl)
        {
            ...
            foreach (Control c in parentControl.Controls)
            {
                if (c.GetType() == typeof(Neusoft.FrameWork.WinForms.Controls.NeuTextBox)
                    || c.GetType() == typeof(Neusoft.FrameWork.WinForms.Controls.NeuComboBox))
                {
        ...
        private void c_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (this.IsDesignMode)
            {
                if (e.KeyCode == Keys.Escape)
                {
                    ...
                    if (this.propertyform != null) this.propertyform.propertyGrid1.Refresh();
                }
            }
        }
        ...
        private void ShowProperty()
        {
            if (propertyform == null)
            {
                propertyform = new frmProperty(this);
                propertyform.Closing += new CancelEventHandler(propertyform_Closing);
            }

            propertyform.Text = "设置-" + XmlFileName;
            propertyform.TopMost = true;
            propertyform.isUseDefault = this.IsUseDefaultSetting;
            if (this.IsShowPropertyForm)
            {
                propertyform.Show();
            }
            ...
            {
                propertyform.ShowNotify(true);
            }
            this.panelMain.Select();
        }
        #endregion
        ...
        {
            if (propertyform != null)
                propertyform.ControlToChange = this.panelMain;
        }
