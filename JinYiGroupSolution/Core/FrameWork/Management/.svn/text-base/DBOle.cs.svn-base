...
namespace Neusoft.FrameWork.Management 
{
	...
	/// wolf 2004-6
	/// </summary>
	public class DBOle:Neusoft.FrameWork.Models.NeuManageObject,IDataBase
	{
		...
		protected System.Data.OleDb.OleDbDataReader TempReader1;
		protected System.Data.OleDb.OleDbDataReader  TempReader2;
		...
		public DBOle(string errLogoFileName) 
		{
		...
		private void newDatabase(string errLogoFileName) 
		{
			...
			this.command.Connection=con as OleDbConnection;
		...
		public void SetTrans(IDbTransaction Trans) 
		{
			try 
			{
				this.command.Transaction = (System.Data.OleDb.OleDbTransaction)Trans;
			}
			catch(Exception ex) 
			{
		...
		{
			this.Logo.WriteLog(this.GetType().ToString()+":"+this.Err);
			//			try
			//			{
		...
		/// 命令 私有
		/// </summary>
		protected System.Data.OleDb.OleDbCommand command;
		/// <summary>
		/// 日志 私有
		/// </summary>
		/// <summary>
		...
		public int Connect(string strConnectString) 
		{
			try 
			{
				...
				con=new  OleDbConnection(strConnectString);
				con.Open();

				Neusoft.FrameWork.Management.Connection.Instance = con as OleDbConnection;//传给全局连接
		...
		/// <param name="strSql">执行sql语句</param>
		/// <returns>0 success -1 fail</returns>
		public int ExecQuery(string strSql,ref OleDbDataReader Reader) 
		{
			//this.command=new OracleCommand();
			a:
				this.command.Connection=this.con as OleDbConnection;
		...
		public int ExecQuery(string strSql,ref DataSet DataSet) 
		{
			...
			try 
			{
				OleDbDataAdapter adapter = new OleDbDataAdapter(this.command);
		...
		public string ExecSqlReturnOne(string strSql,string defaultstring) 
		{
            ...
            if (this.ExecQuery(strSql, ref this.TempReader1) == 0) 
			{
				...
				{
					string strReturn;
					if(((System.Data.OleDb.OleDbDataReader)this.TempReader1).HasRows==false)return defaultstring;
		...
		public int InputBlob(string strSql,byte[] ImageData) 
		{
			...
			OleDbParameter param = command.Parameters.Add(strParam, OleDbType.LongVarBinary);
		...
		public int InputLong(string strSql,string data) 
		{
			...
			strParam=strSql.Substring(i+1,1);
			OleDbParameter param = command.Parameters.Add(strParam, OleDbType.LongVarChar);
			param.Direction = System.Data.ParameterDirection.Input;
		...
		public int ExecEvent(string strSql,ref string Return) 
		{
			...
			try 
			{
				...
				for(int i=1;i<prcParams.GetUpperBound(0);i=i+4) 
				{
					...
					param.OleDbType=(OleDbType)int.Parse(prcParams[i+1]);
					...
					else if(param.OleDbType==OleDbType.VarChar) 
					{
						param.Size=50;
					}
		...
		public IDbConnection con
		{
			...
			set
			{
				...
				this.con1 = value as OleDbConnection;
			}
		}
