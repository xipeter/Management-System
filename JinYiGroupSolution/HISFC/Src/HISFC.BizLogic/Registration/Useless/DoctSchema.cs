			...
			try
			{
				while(this.Reader.Read())
				{
					...
					if(Reader.IsDBNull(4)==false)
						noon.BeginTime=Neusoft.FrameWork.Function.NConvert.ToDateTime(Reader[4].ToString());//开始时间
					if(Reader.IsDBNull(5)==false)
						noon.EndTime=Neusoft.FrameWork.Function.NConvert.ToDateTime(Reader[5].ToString());//结束时间
					if(Reader.IsDBNull(6)==false)
						noon.IsUrg=Neusoft.FrameWork.Function.NConvert.ToBoolean(Reader[6].ToString());//是否急诊
