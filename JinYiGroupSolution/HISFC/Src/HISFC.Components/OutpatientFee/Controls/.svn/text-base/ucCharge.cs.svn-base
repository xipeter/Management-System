...
namespace Neusoft.HISFC.Components.OutpatientFee.Controls
{
    ...
    ///  />
    /// </summary>
    public partial class ucCharge : Neusoft.FrameWork.WinForms.Controls.ucBaseControl, Neusoft.HISFC.BizProcess.Interface.FeeInterface.ISIReadCard, Neusoft.FrameWork.WinForms.Forms.IInterfaceContainer
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        /// <summary>
        /// 是否终端划价 True终端划价使用 False门诊收费使用
        /// </summary>
        private bool isTermianlCharge = false;
        #endregion
        ...
        [Category("控件设置"), Description("是否终端划价 True终端划价使用 False门诊收费使用")]
        public bool IsTermianlCharge
        {
            get
            {
                return isTermianlCharge;
            }
            set
            {
                isTermianlCharge = value;
            }
        }
        #endregion
        ...
        protected virtual void ucShow_SelectedPatient(Neusoft.HISFC.Models.Registration.Register register)
        {
            ...
            #region 判断当前科室是否是执行科室
            //{7749818E-26FF-4cb6-92F2-9F1CE0261BC3}
            ArrayList feeItemLists = null;
            if (!isTermianlCharge)
            {
                feeItemLists = this.outpatientManager.QueryChargedFeeItemListsByClinicNO(register.ID);
            }
            else
            {
                string execDeptID = (outpatientManager.Operator as Neusoft.HISFC.Models.Base.Employee).Dept.ID;
                feeItemLists = this.outpatientManager.QueryChargedFeeItemListsByClinicNOExecDept(register.ID, execDeptID);
            }
            #endregion
        ...
        {
            //zhangyt 2011-03-01
            ZZlocal.Clinic.WinForms.Report.Finance.FinReg.ucClinicFeeDetail ucFeeDetail = new ZZlocal.Clinic.WinForms.Report.Finance.FinReg.ucClinicFeeDetail();
            ZZlocal.Clinic.WinForms.Report.Finance.FinOpb.FeeApplicationPrint feeApplication = new ZZlocal.Clinic.WinForms.Report.Finance.FinOpb.FeeApplicationPrint();
            ...
            ArrayList feeDetails = this.itemInputControl.GetFeeItemList();
            ...
            {
                int result = feeIntegrate.GetAccountVacancy(this.registerControl.PatientInfo.PID.CardNO, ref vacancy, ref accountNO);
                if (result == -1)
                {
                    MessageBox.Show("查询患者账户信息失败！" + feeIntegrate.Err);
                    ((Control)this.registerControl).Focus();
                    return -1;
                }
                ...
                if (result >= 1)
                {
                    ...
                    foreach (FeeItemList f in feeDetails)
                        {
                            fTotCost += f.FT.TotCost;
                        }
                    //}
                    //存在账户并且余额足够
                    if (vacancy >= fTotCost)
                    {
                        bool resultbl = feeIntegrate.CheckAccountPassWord(this.registerControl.PatientInfo);
                        if (!resultbl)
                        {
            ...
            //foreach (ArrayList temp in feeDetails)
            //{
                 //{F0114A43-2079-40dc-BE1E-04611B59B7D9}郑大账户
                if (accountFlag == 2)
                {

                    returnValue = feeIntegrate.SaveFeeToAccount(this.registerControl.PatientInfo, feeDetails, feeTime, accountNO, ref errText);
            ...
            if (accountFlag == 2)
            {
                msgInfo = Language.Msg("账户收费成功!");
                isFee = true;
                //zhangyt 2011-03-01 用于终端  账户余额足够情况下打印费用单 isTermianlCharge是否终端划价项目
                //ucFeeDetail.ClinicFeeList = outpatientManager.QueryFeeItemListsByClinicNO(this.registerControl.PatientInfo.ID);
                if (this.isTermianlCharge)
                {
                    ucFeeDetail.SetValue(this.registerControl.PatientInfo, feeDetails);
                    ucFeeDetail.Print(null, null);
                }
            }
            else if (accountFlag == 1)
            {
                ...
                isFee = false;       
                feeApplication.SetValue(this.registerControl.PatientInfo, feeDetails);
                feeApplication.Print(null, null);
            }
            else
            {
                msgInfo = Language.Msg("划价成功!");
        ...
        protected virtual int LoadPulgIns()
        {
            ...
            try
            {
                this.registerControl = this.feeIntegrate.GetPlugIns<Neusoft.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientInfomation>
                    (Neusoft.HISFC.BizProcess.Integrate.Const.INTERFACE_REGINFO, new ucPatientInfo());
        ...
        protected virtual void popFeeControl_FeeButtonClicked(ArrayList balancePays, ArrayList invoices, ArrayList invoiceDetails, ArrayList invoiceFeeDetails)
        {
            ...
            if (this.medcareInterfaceProxy.IsUploadAllFeeDetailsOutpatient)
            {
                ...
                this.feeIntegrate.PrintInvoice(invoicePrintDll, this.registerControl.PatientInfo, invoices, invoiceDetails, comFeeItemLists,invoiceFeeDetails, balancePaysClone, false, ref errText);
        ...
        protected virtual int InitRightControl()
        {
            ...
            this.rightControl.RealCostEnter += new Neusoft.HISFC.BizProcess.Integrate.FeeInterface.DelegateRealCostEnter(rightControl_RealCostEnter);
            return 1;
        }

        void rightControl_RealCostEnter()
        {
            (this.registerControl as Control).Focus();
        }
