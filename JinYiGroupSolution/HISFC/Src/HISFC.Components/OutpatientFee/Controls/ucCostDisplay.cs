    ...
    public partial class ucCostDisplay : UserControl, Neusoft.HISFC.BizProcess.Integrate.FeeInterface.IOutpatientOtherInfomationRight
    {
            ...
            params string[] otherInfomations) 
        {
            ...
            if (ft != null)
            {
                ...
                decimal tempOwnCost = Neusoft.FrameWork.Function.NConvert.ToDecimal(this.tbOwnCost.Text);
                this.tbRealOwnCost.Text = (tempOwnCost - ft.DayLimitCost).ToString();
                this.tbOwnCost.Text = (tempOwnCost - ft.DayLimitCost).ToString();
            }
        }
        ...
        {
            this.tbRealOwnCost.Focus();
            this.tbRealOwnCost.SelectAll();
        }

        #endregion

        private void tbRealOwnCost_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {
                decimal realCost = Neusoft.FrameWork.Function.NConvert.ToDecimal(this.tbRealOwnCost.Text);
                decimal totCost = Neusoft.FrameWork.Function.NConvert.ToDecimal(this.tbOwnCost.Text);
                if (realCost < totCost)
                {
                    MessageBox.Show("实收金额不能小于应收金额，请重新输入！");
                    this.tbRealOwnCost.Focus();
                    this.tbRealOwnCost.SelectAll();
                    return;
                }
                this.tbReturnCost.Text = (realCost - totCost).ToString();
                if (RealCostEnter != null)
                {
                    RealCostEnter();
                }
            }
