...
{
    public partial class ucQuery2 : UserControl
    {
        public ucQuery2()
        {
            InitializeComponent();
        ...
        Neusoft.HISFC.BizLogic.HealthRecord.Base managerCondition = new Neusoft.HISFC.BizLogic.HealthRecord.Base();
        #endregion
        ...
        {
            return GetWhereString(true,"",true);
        ...
        /// </summary>
        /// <returns></returns>
        public string GetWhereStirngNoUpdate()
        {
            return GetWhereString(false,"",true);
        ...
        /// </summary>
        /// <returns></returns>
        public string GetWhereStirngNoUpdate(string table)
        {
            return GetWhereString(false, table, true);
        ...
        /// <returns></returns>
        public string GetWhereStringSecondTable(string table)
        {
            return GetWhereString(false, table, false);
        }

        /// <summary>
        /// 是否默认保存查询条件 如果多条全部更新
        /// </summary>
        /// <param name="isUpdateCondition"></param>
        private string GetWhereString(bool isUpdateCondition,string table,bool isFirstTable)
        {
            string strWhere = " ";
            for (int i = 0; i < 20; i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    switch (j)
                    {
                        case 0:
                            ...
                            if (isFirstTable)
                            {
                                sCondition = table + ((Neusoft.FrameWork.WinForms.Controls.NeuComboBox)c).SelectedItem.ID;
                            }
                            else
                            {
                                if (!string.IsNullOrEmpty(((Neusoft.FrameWork.WinForms.Controls.NeuComboBox)c).SelectedItem.User01))
                                {
                                    sCondition = table + ((Neusoft.FrameWork.WinForms.Controls.NeuComboBox)c).SelectedItem.User01;
                                }
                            }
                        ...
                        case 1:
                            ...
                            //if (string.IsNullOrEmpty(sOperation))
                            //{
                            //    sOperation = " = ";
                            //}
                            break;
        ...
        /// 根据ID找查询条件
        /// </summary>
        public void ReadConditionByID(string ID)
        {
            string s = "";
            if (this.FindForm() == null) return;
            try
            {
                s = this.managerCondition.GetQueryCondtionByID(ID);
            }
            catch { return; }
            if (s == "-1")
            {
        ...
        /// <returns></returns>
        public ArrayList QueryConditions(string formName)
        {
            return this.managerCondition.GetQueryCondtionInfo(formName);
        }
        /// <summary>
        /// 保存条件(如果有则更新)
        /// </summary>
        ...
        /// 保存条件 
        /// </summary>
        /// <param name="isBool"> true不保存当前操作员 false保存当前操作员</param>
        public void SaveCondtion(string formName, bool isBool)
        {
            string s = this._SaveCondition();
            if (this.managerCondition.InsertQueryConditon(formName, s, isBool,"") == -1)
            {
                MessageBox.Show(managerCondition.Err);
            }
        }
        ...
        public int SaveCondtion(string formName, bool isBool,string condtionName)
        {
            string s = this._SaveCondition();
            if (this.managerCondition.InsertQueryConditon(formName, s, isBool, condtionName) == -1)
            {
                MessageBox.Show("保存失败!"+managerCondition.Err);
        ...
        /// </summary>
        /// <param name="ID"></param>
        /// <param name="condtionName"></param>
        /// <param name="xml"></param>
        public int UpdateCondtion(string ID, string condtionName)
        {
            string s = this._SaveCondition();
            if (this.managerCondition.UpdateQueryCondition(ID, condtionName,s) == -1)
            {
                MessageBox.Show("更新条件设置失败!"+managerCondition.Err);
        ...
        /// </summary>
        /// <param name="ID"></param>
        public int DeleteCondtion(string ID)
        {
            if (this.managerCondition.DeleteQueryCondition(ID) == -1)
            {
                MessageBox.Show("删除失败!"+managerCondition.Err);
        ...
        #region 彩旦
        //private int iLoop = 0;
        protected virtual void panel1_DoubleClick(object sender, System.EventArgs e)
        {
            //if (iLoop > 10)
            //{
            //彩个毛蛋！！
