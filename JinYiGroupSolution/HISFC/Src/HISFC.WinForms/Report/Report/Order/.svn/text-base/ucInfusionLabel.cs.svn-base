        ...
        public object ControlValue
        {
            ...
            set
            {
                ...
                foreach (Neusoft.HISFC.Models.Order.ExecOrder info in alInfusionData)
                {
                    ...
                    if (iIndex == 1)
                    {
                        this.fpSpread1_Sheet1.Cells[0, 0].Text = this.lbBed.Text + " " + (this.lbName.Text.Length > 3 ? this.lbName.Text.Substring(0, 3) : this.lbName.Text)
                    ...
                    this.fpSpread1_Sheet1.Cells[iIndex, 0].Text = ((this.pageNO-1)*4+iIndex).ToString() + "." + info.Order.Item.Name;         //药品名称
                    #endregion
                    //用量 因为存在合并列 列索引+1
