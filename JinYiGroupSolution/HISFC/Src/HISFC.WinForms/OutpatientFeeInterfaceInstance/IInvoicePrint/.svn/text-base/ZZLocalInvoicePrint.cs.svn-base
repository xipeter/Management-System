...
namespace OutpatientInvoicePrint
{
    ...
    ///	修改时间='2011-03-01'
    ///	修改目的='郑大需求'
    ///	修改描述='由于门诊医生站上线，门诊采用电子处方，收费处发票版本更新，每张发票显示8条收费大类，
    ///	           作废3个小联，而上线后门诊依然会有一些手工处方的存在，这时候收费处依然要保留手工发票的
    ///	           版本，此版本为手工发票，ZZLocalNewInvoicePrint为新版本发票。由于发票打印时是反射进OutpaientFeeInterfaceInstance.dll
    ///	           中找继承IInvoicePrint的UC 所以注释掉老发票（此）版本，需要回复此发票版本时只需要取消一次所有的注释，再将ZZLocalNewInvoicePrint注释掉即可
    ///	           上线后手工发票窗口保留以前版本的程序，需要处理手工方时进入老程序（注意将UPDATE.EXE 改成HIS.EXE）。'
    /// />
	/// </summary>
    public class ZZLocalInvoicePrint : System.Windows.Forms.UserControl, Neusoft.HISFC.BizProcess.Interface.FeeInterface.IInvoicePrint, Neusoft.FrameWork.WinForms.Forms.IInterfaceContainer
	{
        #region windows代码
